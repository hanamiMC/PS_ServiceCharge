...
namespace PS_ServiceCharge
{
    public class Gstr
    {
        ...
        public static string FncSetYear(string lvYear)
        {
            string lvReturn = "";

            lvReturn = (fncToInt(Left(lvYear, 2)) + 2500) + "/" + Right(lvYear, 2);

            return lvReturn;
        }

        public static string FncChangeFormatDate(string lvDate,string lvFormat)
        {
            string lvReturn = "";

            try
            {
                DateTime lvDateNew = DateTime.Parse(lvDate + " 00:00:00");
                lvReturn = lvDateNew.ToString(lvFormat);
            }
            catch
            {
